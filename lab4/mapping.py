...
class Mapping:
    ...
    def update_map(self, grid_map, pose, scan):
        ...
        For E: 
            ...
            You should use:# Define the request
            nav_msgs/Path path
            ---
            # Define the response
...
"""  
        ...
        pos = [pose.pose.position.x - origin.position.x, pose.pose.position.y - origin.position.y]
        
        for i,r in enumerate(scan.ranges):
            if r <= scan.range_min or r >= scan.range_max:
                continue
            angle = scan.angle_min + scan.angle_increment*i + robot_yaw
            x = int(((r * cos(angle)) + pos[0]) / resolution)
            y = int(((r * sin(angle)) + pos[1]) / resolution)
            coordinates.append([x,y])
        
        for coordinate in coordinates:
            if not self.is_in_bounds(grid_map, coordinate[0], coordinate[1]):
                continue
            self.add_to_map(grid_map, coordinate[0], coordinate[1], self.occupied_space)
